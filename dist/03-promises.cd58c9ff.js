...
function showPromise(promise) {
    promise.then(({ position , delay  })=>{
        (0, _notiflix.Notify).success(` Fulfilled promise ${position} in ${delay}ms`);
    }).catch(({ position , delay  })=>{
        (0, _notiflix.Notify).failure(` Rejected promise ${position} in ${delay}ms`);
    });
}
...
},{"notiflix":"5z0Oc"}],"5z0Oc":[function(require,module,exports) {
var global = arguments[3];
